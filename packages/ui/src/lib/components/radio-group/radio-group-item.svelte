...
<script lang="ts">
	...
	let { children: userChildren, class: className, ...rest }: RadioGroupItemProps = $props();

	const { slots } = getRadioGroupContext();
</script>

<RadioGroupPrimitive.Item class={cn(slots.item(), className)} {...rest}>
	{#snippet children(args)}
		{#if rest.asChild}
			{@render userChildren?.(args || ({ props: {} } as any))}
		{:else}
			<span class={slots.indicator()}>
				{#if args?.checked}
					<LucideCircle class="radio-group__indicator-icon" />
				{/if}
			</span>
		{/if}
	{/snippet}
